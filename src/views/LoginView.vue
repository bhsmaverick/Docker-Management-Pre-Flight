<template>
  <div class="min-h-screen flex items-center justify-center bg-zinc-950 p-6 font-sans">
    <div class="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-2xl p-8 md:p-10 shadow-2xl relative overflow-hidden">
      ...
      <h2 class="text-3xl font-semibold tracking-tight text-white mb-8 text-center">{{ $t('auth.login_heading') }}</h2>
      ...
      <form @submit.prevent="handleLogin" class="space-y-6">
        ...
        >
          <span v-if="!isLoading">{{ $t('auth.submit_login') }}</span>
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (data.token) {
        // Securely integrate with Pinia or localStorage depending on persistence strategy
        localStorage.setItem('docker_panel_jwt', data.token);
    } else {
        throw new Error('Token payload structural failure');
    }
  } catch (err) {
