...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Fake API call since we don't have the Go backend running on port 3000 mapping
      await new Promise(resolve => setTimeout(resolve, 800));
      
      if (email && password) {
        localStorage.setItem('docker_panel_jwt', 'fake-jwt-token');
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6">
      <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-2xl p-8 md:p-10 shadow-2xl relative overflow-hidden">
        ...
        <h2 className="text-3xl font-semibold tracking-tight text-white mb-8 text-center">{t('auth.login_heading')}</h2>
